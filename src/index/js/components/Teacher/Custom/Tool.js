...
class Tool extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            main: this.props.Teacher.TeacherCustomData.ToolData,
            alter1: this.props.Teacher.TeacherCustomData.ToolAlterData,
    ...
    componentWillReceiveProps(nextProps) {
        ...
        this.setState({
            main: Teacher.TeacherCustomData.ToolData,
            //main: Teacher.TeacherCustomData.ToolAlterData,
            alter1: Teacher.TeacherCustomData.ToolAlterData,
        })
    }
    // 从id选择列表
    id2List = {
    ...
    onDragEnd = result => {
        ...
        if (source.droppableId === destination.droppableId && source.droppableId === 'main') {//同一个区域
            ...
            dispatch(TeacherCustomActions.fetchCustomData('/SubjectResMgr/ToolMgr/Teacher/EditDeskTop','tool'))
    ...
    onEditClick = (source) => {
        ...
        dispatch(TeacherCustomActions.setCustomData('alter', dataObj, source, destination))
        dispatch(TeacherCustomActions.fetchCustomData('/SubjectResMgr/ToolsMgr/Teacher/EditDeskTop','tool'))
    }
    ...
    onDeleteClick = (dataObj, source, ID) => {
        ...
        dispatch(TeacherCustomActions.fetchDeleteCustomData('/SubjectResMgr/ToolMgr/Teacher/DeleteTool', ID,'tool'))
    }
    // card编辑
    onResetClick = (source) => {
        const { Teacher, dispatch } = this.props
        ...
        return (
            <Loading opacity={false} spinning={AppLoading.customOpacityLoading}>
                <Loading spinning={AppLoading.customLoading}>
                    <div id='Tool'>
                        <div style={{ display: Teacher.TeacherCustomData.TipsShow.ToolTipsShow ? 'block' : 'none' }} className='my-Tips'>
                            <div className='tips-left'>
                                <i className='tips-icon'></i>
                                <span className='tips-text'>操作提示: 拖放图标可调整工具的摆放顺序或进行分组。</span>
                            </div>
                        ...
                        <DragDropContext onDragEnd={this.onDragEnd}>
                                                        ...
                                                        <Draggable draggableId={'main-' + item.ID} index={index}>
                                                            {(provided, snapshot) => (
                                                                <Card
                                                                    ...
                                                                    provided={provided}
                                                                    snapshot={snapshot}
                                                                    onDeleteClick={this.onDeleteClick.bind(this, { ToolData: MainData }, { droppableId: 'main-' + item.ID, index: index }, item.ID)}
                            ...
                            <div className='changeBox'>
                                <span className='box-tips'>备选工具<span className='tips-1'>（共<span className='tips-2'>{this.getNumber(AlterData)}</span>个）</span></span>
                            </div>
                                                        ...
                                                        <Draggable key={'alter' + index + '-' + item.ID} draggableId={'alter' + index + '-' + item.ID} index={index1}>
                                                            {(provided, snapshot) => (
                                                                <Card
                                                                    ...
                                                                    onDeleteClick={this.onDeleteClick.bind(this, { ToolAlterData: AlterData }, { droppableId: "alter" + index, index: index1 }, item.ID)}
