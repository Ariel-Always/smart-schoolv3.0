...
class App extends React.Component {
    ...
    onDragEnd = result => {
        ...
        if (source.droppableId === destination.droppableId && source.droppableId === 'main') {//同一个区域
            ...
            dispatch(TeacherCustomActions.fetchCustomData('/SubjectResMgr/AppMgr/Teacher/EditDeskTop','App'))
    ...
    onDeleteClick = (dataObj, source, ID) => {
        ...
        dispatch(TeacherCustomActions.fetchDeleteCustomData('/SubjectResMgr/AppsMgr/Teacher/DeleteAppInfo', ID,'App'))
    }
    // card编辑
    onResetClick = (source) => {
        const { Teacher, dispatch } = this.props
        ...
        return (
            <Loading opacity={false} spinning={AppLoading.customOpacityLoading}>
                <Loading spinning={AppLoading.customLoading}>
                    <div id='app'>
                        <div style={{ display: Teacher.TeacherCustomData.TipsShow.AppTipsShow ? 'block' : 'none' }} className='my-Tips'>
                            <div className='tips-left'>
                                <i className='tips-icon'></i>
                                <span className='tips-text'>操作提示: 拖放图标可调整应用的摆放顺序或进行分组。</span>
                            </div>
                        ...
                        <DragDropContext onDragEnd={this.onDragEnd}>
                                                        ...
                                                        <Draggable draggableId={'main-' + item.ID} index={index}>
                                                            {(provided, snapshot) => (
                                                                <Card
                                                                    ...
                                                                    provided={provided}
                                                                    snapshot={snapshot}
                                                                    onDeleteClick={this.onDeleteClick.bind(this, { AppData: MainData }, { droppableId: 'main-' + item.ID, index: index }, item.ID)}
                            ...
                            <div className='changeBox'>
                                <span className='box-tips'>备选应用<span className='tips-1'>（共<span className='tips-2'>{this.getNumber(AlterData)}</span>个）</span></span>
                            </div>
                                                        ...
                                                        <Draggable key={'alter' + index + '-' + item.ID} draggableId={'alter' + index + '-' + item.ID} index={index1}>
                                                            {(provided, snapshot) => (
                                                                <Card
                                                                    ...
                                                                    onDeleteClick={this.onDeleteClick.bind(this, { AppAlterData: AlterData }, { droppableId: "alter" + index, index: index1 }, item.ID)}
