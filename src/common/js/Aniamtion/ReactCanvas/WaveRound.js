...
class WaveRound extends React.Component {
  ...
  componentDidMount() {
    ...
    if (canvas.getContext) {
      ...
      // console.log('wode')
      this.update(ctx, vertexes, diffPt);
  ...
  update = (ctx, vertexes, diffPt) => {
    ...
    this.draw(ctx, vertexes);
    let requestAnimaFrame =  window.requestAnimaFrame(() => this.update(ctx, vertexes, diffPt));
    this.setState({
      diffPt: diffPt,
      vertexes: vertexes,
      // xx: xx,
  ...
  draw = (ctx, vertexes) => {
    ...
    for (let j = 0; j < len; j++) {
      ...
      ctx.lineTo(
        vertexes[0].x + j * deviation,
        vertexes[0].y 
      );
      for (let i = 1; i < vertexes.length; i++) {
        ctx.lineTo(
          vertexes[i].x + j * deviation,
          vertexes[i].y 
        );
      }
  ...
  render() {
    return (
      <canvas ref={this.canvas} className={"WaveRound "+ this.state.background}>
