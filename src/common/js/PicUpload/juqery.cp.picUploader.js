(function ($) {
    ...
    $.fn.extend({
        ...
        "picUploader": function (options) {
            var html = '<div class="up_area '+(options.size?options.size:"big")+'">'
                ...
                + '				<p>上传要求:1寸照片,大小≤2MB,分辨率≥358px*441px,限JPG/JPEG/GIF/BMP/PNG格式。</p>'
                + '			</div>'
                + '		</div>'
                + '</div>'
                ...
                + '     <div class="modal_dialog" style="width:650px;height:508px;">'
                ...
                $.ajax({
                    ...
                    success: function (data, textStatus, jqXHR) {
                        if (data.Status === 200) {
                            ...
                            $('#up_img_preview').css('background-image', 'url(' + _up_obj.resWebUrl + data.Data[0] + '?t=' + timestamp.toString() + ')').show();
            ...
            $('#up_btn_default').click(function () {
                ...
                $('.up_img_current').hide();
                $('#up_img_preview').css('background-image', 'url(' + perview + ')').show();
            });
            ...
            if (_up_obj.crop) {
                ...
                $(".up_tip_container p").text("上传要求:1寸照片,限JPG/JPEG/GIF/BMP/PNG格式。");
            }
    ...
    function _userPhoto_checkFileSize(evt) {
        if (evt.target === undefined || evt.target === null || evt.target.files === null) {
            return false;
        }
    ...
    function _userPhoto_getDefaultPhoto() {
        if (_up_obj.userType === "Admin") {
            return _up_obj.resWebUrl + 'UserInfo/Photo/Default/Nopic001.jpg';
        }

        if (_up_obj.gender === '-1') {
            return _up_obj.defultPreImgPath;
        }
        ...
        var photoUrl = _up_obj.resWebUrl + 'UserInfo/Photo/Default/Nopic' + int_userType + _up_obj.gender + '1.jpg';
        return photoUrl;
    }
    ...
    function _userPhoto_reset(options) {
        ...
        _up_obj.defultPreImgPath = _up_obj.resWebUrl + 'UserInfo/Photo/Default/Nopic.jpg';
        ...
        if (_up_obj.curImgPath === '') {
            $(".up_img_current").attr('data-src', _up_obj.defultPreImgPath).css('background-image', 'url(' + _up_obj.defultPreImgPath + ')');
            _userPhoto_setUsingDefaultVal(true);
        } else {
            $(".up_img_current").attr('data-src', _up_obj.curImgPath).css('background-image', 'url(' + _up_obj.curImgPath + ')');
        }
    }
    ...
    function _userPhoto_setCurImgPath(imgUrl, userID, userType) {
        ...
        $(".up_img_current").attr("data-src", src).css("background-image", 'url(' + src + ')');
